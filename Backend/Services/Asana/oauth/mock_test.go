...
func TestCallBack(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedURL:  "http://localhost:8081/login?asana_code=valid_code",
		},
		{
			name:         "Missing code",
			queryParams:  "",
...
func TestOAuthFront(t *testing.T) {
	...
	t.Run("Get OAuth URL", func(t *testing.T) {
		...
		assert.Contains(t, w.Body.String(), "https://app.asana.com/-/oauth_authorize")
		assert.Contains(t, w.Body.String(), "response_type=code")
		assert.Contains(t, w.Body.String(), "state=default")
	})
}
