...
func getAction(c *gin.Context, request string) *json.RawMessage {
	...
	prompt := "Tu es un agent pour une application de création d'action et de réaction." +
		...
		"Afin de trouver les bons ids, types et arguments de l'action, tu dois te servir d'une liste de toutes les actions possibles : \n###\n" + string(body) + "\n###\n" +
		"Si dans les arguments il y a un tableau vide ([]) alors ne met rien." +
		"Example: \n###\n{ action_id: 0, action_type: 0, hour: 17, minute: 45 }\n###\n" +
...
func getReaction(c *gin.Context, request string) *json.RawMessage {
	...
	prompt := "Tu es un agent pour une application de création d'action et de réaction." +
		...
		"Afin de trouver les bons ids, types et arguments de la reaction, tu dois te servir d'une liste de toutes les reactions possibles : \n###\n" + string(body) + "\n###\n" +
		"Example: \n###\n[{ reaction_id: 0, reaction_type: 0, hour: 17, minute: 45 }]\n###\n" +
		"Envoie moi SEULEMENT l'objet JSON, je ne veux aucun autre texte."
